...
SCA_IActuator *SCA_PythonController::LinkedActuatorFromPy(PyObject *value)
{
  ...
  /* set the exception */
  PyErr_Format(PyExc_ValueError, "%R not in this python controllers actuator list", value);
...
PyTypeObject SCA_PythonController::Type = {
    PyVarObject_HEAD_INIT(nullptr, 0) "SCA_PythonController",
...
PyAttributeDef SCA_PythonController::Attributes[] = {
    EXP_PYATTRIBUTE_RW_FUNCTION(
        "script", SCA_PythonController, pyattr_get_script, pyattr_set_script),
...
bool SCA_PythonController::Import()
{
  ...
  if (function_string.empty()) {
    CM_LogicBrickError(this,
                       "python module name formatting expected 'SomeModule.Func', got '"
                           << m_scriptText << "'");
    return false;
  }
  ...
  if (m_function == nullptr) {
    if (PyErr_Occurred())
      ...
      CM_LogicBrickError(this,
                         "python module '" << m_scriptText << "' found but function missing");
    return false;
  }
  ...
  m_function_argc =
      0; /* rare cases this could be a function that isn't defined in python, assume zero args */
...
PyObject *SCA_PythonController::PyDeActivate(PyObject *value)
{
  if (m_sCurrentController != this) {
    PyErr_SetString(PyExc_SystemError,
                    "Cannot deactivate an actuator from a non-active controller");
    return nullptr;
  }
...
PyObject *SCA_PythonController::pyattr_get_script(EXP_PyObjectPlus *self_v,
                                                  const EXP_PYATTRIBUTE_DEF *attrdef)
{
  // SCA_PythonController* self =
  // static_cast<SCA_PythonController*>(static_cast<SCA_IController*>(static_cast<SCA_ILogicBrick*>(static_cast<EXP_Value*>(static_cast<EXP_PyObjectPlus*>(self_v)))));
...
int SCA_PythonController::pyattr_set_script(EXP_PyObjectPlus *self_v,
                                            const EXP_PYATTRIBUTE_DEF *attrdef,
                                            PyObject *value)
{
  SCA_PythonController *self = static_cast<SCA_PythonController *>(self_v);
  ...
  if (scriptArg == nullptr) {
    PyErr_SetString(
        PyExc_TypeError,
        "controller.script = string: Python Controller, expected a string script text");
